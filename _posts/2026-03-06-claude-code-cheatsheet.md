...
> Tip：运行 `/terminal-setup` 可启用多行输入（`Shift+Enter`）；在 iTerm2 / VS Code 可用 `/keybindings` 自定义快捷键。
...
- What：你 own 的`/commands`,手动调用的自定义提示词模板（Markdown 文件）
...
> Permissions Example
>- 例如将 `Bash(git:*)` 加到 `allowedTools`，可放行 git 相关命令而不再询问。
