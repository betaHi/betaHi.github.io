---
title: Pi 系列 06｜实战：给应用加一个 tool（customTools 与 extension 两种方式）
date: 2026-06-15 12:00:00 +0800
categories: [Tech, Agent]
tags: [pi, agent-harness, tool, extension, sdk, practice]
---
...
> 给基于 pi 的应用加一个 tool，有 `customTools` 和 extension 两种注册方式。本文以一个查天气的 tool 为例，分别用这两种方式实现。
> tool 定义本身在两种方式下是一样的，区别只在于 tool 如何进入系统。

## 问题：模型查不了天气

加工具之前，询问「北京天气怎么样」，它会这样回答：“我这边不能实时查询天气数据。你可以看一下手机天气 App 或搜索"北京天气"获取最新情况。”

![没有工具时，模型让你自己去查](/assets/img/2026-06-15_2.png)

这不是模型不愿意回答，而是它**没有这个能力**。模型本身只能根据上下文生成文本，读不了实时数据；天气是实时的、训练数据里没有，所以只能让你自己去查。
...
- **`noExtensions: true` 只关掉「默认/配置发现的 extension」，不影响内联的 `extensionFactories`**。所以即使设了 `noExtensions: true`，这个内联 extension 仍然会执行。（实测验证过：这样注册的工具在 `noTools:'builtin'` 下同样会被激活、暴露给模型、成功调用。）

## 两种方式的结果一样

无论走哪条，问「北京今天天气怎么样」，模型都会生成 `get_weather({ city: "北京" })`，pi 执行、真实天气回上下文、模型据此回答。

![加了工具后，模型查到了真实天气](/assets/img/2026-06-15_3.png)
