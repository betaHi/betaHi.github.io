...
从 harness engineering 的角度看，这一章有几个很值得学的点。

#### 1. 长会话管理一定要显式引入边界

只做摘要、不做边界，系统后面几乎一定会出现状态混乱。

#### 2. 历史压缩要服务 runtime 连续性，而不是文档归档

agent runtime 关心的是“还能不能继续正确干活”，不是“能不能完整回忆整段对话”。

#### 3. summarize、memory、compact 最好是三种不同层级

- summarize：内容变短
- compact：会话历史收缩与边界化
- memory：长期上下文持久化

Claude Code 当前最可取的地方，就是大体上把这三者分开了。

#### 4. 恢复系统应围绕当前有效现实设计

压缩后就承认进入了新阶段，后续恢复围绕新阶段做，而不是执着于完美回放旧世界。

---

### 十二、工程化启发

#### 1. 做 agent runtime 时，压缩机制一定要和状态机边界绑定

不要把压缩做成纯文本处理函数；要把它接入 query/runtime 的结构边界。

#### 2. 保留“当前可工作”比保留“全部原始内容”更重要

在有限上下文下，这是更现实也更稳定的目标。

#### 3. 对压缩后的世界要重新定义 canonical history

否则后续模块会各自理解不同的“当前历史”。

#### 4. compact 之后的恢复应被视为正常路径，而不是异常兜底

Claude Code 的架构脉络说明，这种思路更适合长会话 agent。
