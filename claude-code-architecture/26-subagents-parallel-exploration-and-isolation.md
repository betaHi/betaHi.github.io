...
`Plan`、`code-reviewer` 之类类型说明，Claude Code 甚至把某些高层工作流本身也托管给专门子代理，这进一步说明子代理系统是一个横跨多层任务类型的执行框架。
...
#### 1. 真正成熟的 agent runtime 应支持“委托”，但更应支持“隔离”

很多系统只关注并行，却低估了上下文污染问题。Claude Code 的子代理设计更像是在优先解决污染，再顺带获得并行收益。

#### 2. 子代理应按角色建模，而不是全是同质 worker

Explore、Plan、Reviewer 这种角色化设计很实用，因为它能把任务边界显式化。

#### 3. 需要明确规定主代理不能放弃理解责任

“Never delegate understanding” 其实是整个多代理系统避免失控的关键原则。

#### 4. 文件系统隔离是多代理代码执行的关键增强

如果只做 prompt 隔离，不做 worktree 级隔离，多代理修改代码时很容易互相踩踏。

---

### 十四、工程化启发

#### 1. 如果 agent 要处理开放式探索，最好提供子代理而不是无限扩大主上下文

这是更稳、更便宜、也更可控的路线。

#### 2. 多代理系统应把“角色”和“隔离级别”都做成一等概念

不仅要问“派不派子代理”，还要问：

- 这是探索型、规划型还是审查型？
- 只需上下文隔离，还是需要 worktree 隔离？

#### 3. 子代理返回结果最好是压缩过的工作结论，而不是完整过程倾倒

这样主代理才能维持综合能力而不被细节淹没。

#### 4. 主代理应始终保留路由与综合权

否则多代理系统很容易退化成多个互不负责的局部执行体拼盘。
