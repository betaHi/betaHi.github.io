...
从 harness engineering 的角度看，这一章有几个特别值得学的点。

#### 1. 真正可工作的 agent 不应只会同步单轮响应
...
任务描述“做什么”，执行进程处理“怎么跑”。这让系统更清晰，也更容易恢复和展示。

#### 3. 异步系统要把回收设计在一开始

启动后台命令很容易，完成后的可追踪消费才是架构难点。

#### 4. 任务基础设施能显著减轻长会话对历史上下文的依赖

这对 compact、恢复、多轮协作都很有帮助。

---

### 十二、工程化启发

#### 1. 为 agent 增加任务层，比单纯增强 prompt 更能提升真实工作能力

因为它改变的是工作组织方式，而不是回答措辞。

#### 2. 后台执行、定时调度、任务追踪最好共享一套会话级任务语义

这样用户和 agent 才能用统一方式理解“有哪些工作正在进行”。

#### 3. 异步结果应落到可引用对象，而不是只在瞬时日志里闪过

Claude Code 用 output file / task output 的方式，给了一个很实用的答案。

#### 4. 任务层要克制，不要把所有小动作都任务化

任务应服务工作协调，而不是制造新的管理负担。
