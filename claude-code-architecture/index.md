...
这本书把 Claude Code 当成一份**以 harness engineering 为视角的架构观察笔记**。它不讲怎么用 Claude Code，不做源码逐行导读，也不是 agent 入门教程。它只做一件事：**把一个真实、成熟、长期演化中的 agent runtime 拆开**——看它把哪些复杂度前置到了架构层，又把哪些边界做成了正式的运行时结构——然后从中提炼可以迁移到其他系统的工程判断。

## 从哪里开始

- 第一次来：先看 [前言](/claude-code-architecture/preface/)
- 想先建立整体感：[00 总览](/claude-code-architecture/00-overview/)
- 想直接看工程判断：[09 Harness 工程经验综合](/claude-code-architecture/09-harness-engineering-lens/)
- 想要总图：[27 总体运行时架构图](/claude-code-architecture/27-runtime-architecture-map/)
