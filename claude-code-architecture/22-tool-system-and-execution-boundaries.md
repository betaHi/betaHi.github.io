...
从 harness engineering 的角度看，Claude Code 的工具系统有几个很值得学的地方。

#### 1. 工具要先是 contract，再是 implementation

很多 agent 系统一开始就只想着“能调用什么函数”，但 Claude Code 这类架构更强调：
...
专门工具、细粒度权限、结构化结果，这些都说明一个核心工程原则：

- 不要因为模型会调用，就把执行面做成失控的万能接口

#### 3. 主循环只应吸收真正属于主路径的执行能力

hooks、memory extraction、post-sampling analysis 没被强行改成 tool，是因为它们不属于同一种 runtime 语义。

这种边界感非常重要。

#### 4. 统一执行接口不等于统一扩展模型

Claude Code 并没有把 skill、hook、tool、MCP 全合并成一个系统。它更像承认：

- 不同能力类型需要不同接缝
- “统一”应该发生在合适层级，而不是全盘抹平差异

---

### 十二、工程化启发

#### 1. agent 的工具系统应该优先解决“能力 contract”问题
...
#### 3. permission system 最好不要做成工具外的松散附加层

Claude Code 这类做法说明，更好的方式是：

- 工具 contract 与治理执行天然耦合
- 而不是“先设计工具，再在外面勉强包一层安全检查”

#### 4. 只把真正需要模型显式决策的能力暴露成 tool

其他运行时机制可以继续放在 hook、prefetch、post-processing、stop phase 等不同层，不必为了形式统一而全部工具化。
